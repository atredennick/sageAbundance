...
Yet, global environmental changes, like climate change, play out at landscape and regional scales, creating a mismatch between available tools and real world problems.
New technology is generating data products on massive scales covering entire ecosystems and teams are post-processing these data products to turn them into useful ecological information like species-specific estimates of plant percent cover.
Unlike some ecological sub-disciplines, population modelers have yet to fully embrace the use of remotely-sensed data as estimates of population abundance.
Such reluctance likely stems from challenges in fitting statistically appropriate models that rigorously account for spatial autocorrelation in large datasets.
Here, we use modern, computationally efficient model reparameterizations to fit a dynamic population model to a remotely-sensed dataset.
We use a 28 year time series of sagebrush (*Artemisia* spp.) percent cover over a $2.5\times5$ km landscape in southwestern Wyoming to fit a spatio-temporal population model that includes climate covariates and spatial random effects.
...
Species distribution models (SDMs), on the other hand, are easily applied at landscape to continental scales [e.g., @Maiorano2013; @Clark2014].
In a climate change context, SDMs typically rely on static associations between contemporary climate and a species' distribution or, more rarely, abundance to project future distribution or abundance [@Elith2009].
Thus, interannual dynamics that reflect population responses to yearly climate variations are not represented in SDMs.
Furthermore, since SDMs typically rely on occurence data, their projections of habitat suitability or probability of occurence provide little information on the future states of populations in the core of their range -- in areas where a species currently exists and is expected to persist in the future.
...
The ideal tool for forecasting future population states under climate change would be a large-scale dynamic population model [@Schurr2012a; @Merow2014].
Unfortunately, such methods have been elusive due to the lack of time series data at large spatial extents and the statistical machinary needed to fit high-dimensional spatial models.
...
RS time series have revolutionized models of ecosystem-level processes [e.g., @Running2004] and have been used to detect long term trends in plant population abundance [e.g., @Homer2015], but they have yet to be used to drive a dynamic population model.
...
However, these methods have yet to be used in a dynamic setting. 

Building on these technological advances, we develop a statistical approach for modeling plant populations over large spatial extents using a process model motivated by Gompertz density-dependent population growth.
Our approach leverages a unique remotely-sensed time series of *Artemisia* (sagebrush) species [@Homer2012] to fit a spatio-temporal model of plant population dynamics at the landscape scale.
...
Sagebrush ecosystems offer an ideal testing ground for our new modeling approach: sagebrush species are widely distributed [@Kuchler1964], sagebrush species are sensitive to climate [@Dalgleish2011; @Schlaepfer2014; @Schlaepfer2014a], and forecasts of future sagebrush ecosystems are in high demand due to the precarious conservation status of the greater sage-grouse (*Centrocercus urophasianus*) [@Arnett2015].
Species distribution models typically predict that much of the area occupied by sagebrush ecosystems today will become unsuitable for sagebrush due to climate change, resulting in a dramatic loss overall in the extent of sagebrush habitat by the end of this century [@Shafer2001; @Neilson2005; @Bradley2010; @Schlaepfer2012; @Still2015].
...
Second, borrowing from the vast literature on spatio-temporal modeling of animal abundance [e.g., @Conn2015], we employed a dimension reduction strategy to rigorously account for spatial autocorrelation.
Accounting for spatial autocorrelation allows for statistically rigorous inference.
While important statistically, in terms of ecological inference, we can be confident in our estimates of climate effects on sagebrush cover change in our study region.
It also provided a way to obtain spatially-explicit predictions at a resolution below that of the climate covariates (Figs. 4,5).
...
Thus, we did find some support for the idea that sagebrush will respond to positively to temperature increases colder parts of their range.
...
The relatively weak climate effects suggests that we should expect small changes when we apply the model to projected climate scenarios.
...
The discrepancy between Homer et al.'s (2015) results and ours is likely due to the climate projections used for forecasting future changes.
...
There is no clear answer to what climate model to use when making forecasts.
...
We used dimension reduction techniques, bu as more and more knots are used, computation time increases substantially [@Wikle2010].
Had we decided to aggregate the original data to a coarser resolution, we would have been able to model a larger spatial extent using the same number of knots without increasing computation time.
...
In other words, climate informatino in our model only varied through time and not over space.
