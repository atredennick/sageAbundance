...
Fortunately, advances in the spatiotemporal modeling of animal abundances offer a scaffold for similar work on plant species.

Spatiotemporal models of animal abundance seek to relate the number of observed counts at location *s* and time *t* ($C_{s,t}$) to a latent process $\mu_{s,t}$ through a probability mass function, a link function relating predictors to the mean response, a set of covarites __X__ and associated parameters $\boldsymbol{\theta}$, and a spatial random effect $\boldsymbol{\eta}$. 
A typical formulation takes the form

\begin{align}
[\textbf{C}] &= \text{Poisson}(\boldsymbol{\mu}) \\
\boldsymbol{\mu} &= \text{exp}(\boldsymbol{\theta}\textbf{X} + \boldsymbol{\eta})
\end{align}

which states that observed counts are Poisson distributed with a log-link function on the latent abundance intensity ($\boldsymbol{\mu}$), which is itself a function of covariates and estimated parameters. 
The exact formulation of the latent process model (Eq. 2) can take many forms, including, for example, those motivated by classic density-dependent growth [@Ross2015] or by resource-selection processes [@Conn2015].
However, all models like Eq. 2 share the need to estimate the spatial random effect ($\boldsymbol{\eta}$).
In some cases this is as straightforward as fitting a random effect for each sampling unit.
But when the number of sampling units is large (1000s), fitting these effects becomes computationally untenable.
Fortunately, modern model reparameterizations and dimension reduction techniques allow for estimation of spatial random effects by mapping a reduced parameter set (which are estimated in the mode) to the full spatial extent under consideration with an interpolation matrix [@Wikle2010; @Conn2015].
The general model outlined by Eqs 1-2, and the associated computational strategies for fitting them, have recieved wide usage in the animal abundance literature [@Fink2010; @Conn2015; @Ross2015], but have yet to be applied to the modeling of plant populations.
This is most likely because the methods are relatively new and because, until now, estimates of plant abundance over large spatial extents have been rare. 

Here we develop a statistical approach for modeling plant populations over large spatial extents based on the above framework and a process model motivated by Gompertz density-dependent growth.
...
One problem with our statistical approach that treats integer cover as counts, is that the Poisson distribution supports values greater than 100, a value that percent plant cover cannot exceed in reality.
Thus, when simulating the fitted model it is entirely possible to predict values of cover greater than 100%.
Likewise, once a pixel reaches such a great value, the exponential function $\text{exp}(\beta_{1} y_{i,t-1})$ (Eq. #) results in runaway growth and eventually all pixels go to unrealistic values of percent cover.
We therefore imposed a rule in our simulations that if predicted cover from a pixel exceeded 100%, it's value was set to the mean value of all pixels at that timestep.
Our main purpose for simulating the model is to assess percent cover at equilibrium, so our focus is not on year-to-year dynamics that are affected by this rule.
The rule simply ensures that percent cover cannot go over 100% while maintaining any mean changes in percent cover since pixels exceeding 100% are set to the mean matrix value.
Admittedly, such a rule is not ideal, but we have chosen practicality over idealism in this case.
...
Acknowledgments
---------------
This work is the outcome of a distributed graduate seminar led by PBA and supported by a National Science Foundation CAREER award (DEB-XXXXXX). ATT was supported by a NSF Postdoctoral Research Fellowship in Biology (DBI-XXXXX) and AK was supported by a NSF Graduate Research Fellowship (DEB-XXXXXX). We are grateful to Mary LASTNAME at USGS EROS for exctracting the dataset used in this paper.
...
```{r figure_4,dependson="plot-options", fig.cap="Projected equilibrium cover from simulations using observed climate (top panel) and average observed percent cover (bottom panel).", fig.height=8, eval=T}
library(png)
library(grid)
...
```{r figure_6,dependson="plot-options", fig.cap="Projected equilibrium cover under climate change scenarios. Top panels shows equilibrium cover based on simulations using observed cliamte. Subsequent paneld show equilibrium cover based on perturbed climate for each RCP scenario.", fig.height=8, eval=T}
library(png)
library(grid)
...
```{r figure_7,dependson="plot-options", fig.cap="Estimated kernel densities for current and projected sagebrush cover across space and simulation time.", fig.height=8, eval=T}
library(png)
library(grid)
