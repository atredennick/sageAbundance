...
The amount of precipitation in the previous year (*pptLag* in our model) emerged as a strong predictor of sagebrush cover change (Fig. 2) which agrees with previous research showing that wetter winters are correlated with greater stem growth in sagebrush [@Poore2009; @Apodaca2013]. The posterior distribution for the effect of spring temperature on sagebrush cover broadly overlapped zero but was slightly positive (Fig. 2).  Warmer spring temperatures may enhance sagebrush growth in colder climates by advancing the date of snowmelt and increasing the length of the growing season [@Perfors2003; @Harte2015].  As seen when comparing our temporally-explicit forecasts (Fig. 6) to climate projections (Fig. 2), it also appears that our forecasts were driven by the weak, but positive, response to temperature. 
...
While we presented projections for sagebrush cover at the end of this century, one important feature of our aproach is that it could also produce short-term forecasts for sagebrush cover as well.  For instance this apprach could be used to model the effects of a multi-year regional drought on this species (cite Debinski et al. 2010).  Validating spatial population models against short-term predictions would give modelers a way to assess the long-term forecasts produced by the model.  It would also provide opportunities for refinement of the model itself.  This cycle of prediction, validation and refinement is missing from most population-level forecasts of the effects of climate change currently produced [@Petchey2015 prediction paper -- is there a better reference for this idea?].   
...
As more species-level remote sensing datasets become available and computing power increases this approach will be applicable to a wider number of species and even larger spatial extents. Future modeling could include the effects of non-climate drivers--including the effects of species interactions and disturbance [@Clark2014].  For sagebrush, including fire and competition with non-native annual grasses in the model may be especially important for a complete assesment of the effects of climate change [@Bradford and Lauenroth  2009 ].  Fortunately, our spatio-temporal modeling framework could easily be extended to model additional species and dynamic processes as the data become availabe. The approach we have developed here fills an important gap in spatial scales between species distribution models and local-scale demographic population models.
