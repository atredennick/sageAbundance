...
Spatiotemporal models of animal abundance seek to relate the number of observed counts at location *s* and time *t* ($C_{s,t}$) to a latent process $\mu_{s,t}$ through a probability mass function, a link function relating predictors to the mean response, a set of covarites __X__, and parameters $\boldsymbol{\theta}$. 
A typical formulation takes the form

\begin{align}
[\textbf{C}] &= \text{Poisson}(\boldsymbol{\mu}) \\
\boldsymbol{\mu} &= \text{exp}(\boldsymbol{\theta}\textbf{X})
\end{align}
...
The covariates include spatial and temporal autocorrelation effects and any number of additional measured environmental effects. 
In a population modeling sense, the temporal effect can be defined as the effect of population abundance at time *t-1*.
More difficult is the definition of the spatial random effects in models that include a large number of locations, such as remote sensing data.
Here too, advances have been made on strategies for dimension reduction that allow for fitting a reduced parameter set that can be expanded to express high-dimensional random effects [@Wikle2010].

Here we develop a statistical approach for modeling plant populations over large spatial extents based on the above framework,.
...
```{r figure_6,dependson="plot-options", fig.cap="Projected equilibrium cover under climate change scenarios. Top panels shows equilibrium cover based on simulations using observed cliamte. Subsequent paneld show equilibrium cover based on perturbed climate for each RCP scenario.", eval=TRUE, fig.height=8}
library(png)
library(grid)
