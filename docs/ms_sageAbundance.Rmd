...
The climate effects estimated in our model, and detected at a spatial resolution of 30 meters, are consistent with individual-level responses of sagebrush to climate-related variables.
At the level of individual plants, research shows that wetter winters are correlated with greater stem growth in sagebrush [@Poore2009; @Apodaca2013] and warmer spring temperatures may enhance sagebrush growth in colder climates by advancing the date of snowmelt and increasing the length of the growing season [@Perfors2003; @Harte2015].
Mirroring those individual-level responses, posterior means for all climate effects in our cover change model were positive, except for the effect of fall-through-spring precipitation one year before a cover transition (*ppt1*, Fig. 3).
...
But, the covariates we chose closely match the climate-related variables that have been shown to drive sagebrush growth, survival, and regeneration.
...
Our forecasts are a direct result of the interaction between the estimated effect size for each cliamte covariate and the amount of change in those covariates projected in the future.
...
An interesting consequence of explicitly modeling the effect of space (through $\boldsymbol{\eta}$) is the increase in spatial heterogeity we forecast: the distribution of sagebrush cover expands rather than just shifting (Fig. D1).
This effect is seen in the spatial forecasts where low cover pixels are constrained to low cover while high cover pixels increase (Fig. 5).
Had we not explicitly accounted for spatial-dependence in our model, this result would be missed.
...
The generally positive effect of precipitation we found is consistent with findings from across the sagebrush distribution [@Schlaepfer2012; @Schlaepfer2015].
However, the effect of temperature may vary depending on location.
...
To overcome this, future investigators modeling a larger spatial extent may need to aggregate their original remote-sensing time series data to a coarse spatial resoluation. This would allow them to model a much greater spatial extent with a similar number of knots and a similar amount of computation time.
