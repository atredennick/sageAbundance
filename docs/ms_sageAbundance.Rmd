...
The major advantage of our approach is that, once fit, the model can be used to project future changes in population abundance (here, percent cover) in space and time.
We applied the fitted model to three IPCC emissions scenarios and forecast modest increases in sagebrush percent cover (Figs. 4, 5).
The forecasted increases are primarily driven by a positive response to precipitation (Fig. 2).
...
The descrepency between Homer et al.'s [@Homer2015] results and ours appears to be due to the climate projections used for forecasting future changes.
...
Our spatiotemporal modeling framework is very flexible.
...
```{r figure_6,dependson="plot-options", fig.cap="Projected equilibrium cover under climate change scenarios. Top panels shows equilibrium cover based on simulations using observed cliamte. Subsequent panels show equilibrium cover based on perturbed climate for each RCP scenario.", fig.height=8, eval=F}
library(png)
library(grid)
...
```{r figure_8,dependson="plot-options", fig.cap="Model forecasts under future climate for three IPCC emissions scenarios. Shaded regions show 90/% quantiles for simulations conducted using different sets of parameters from the MCMC output. Lines show mean trajectories.", eval=F}
library(png)
library(grid)
