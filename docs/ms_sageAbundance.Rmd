...
While we focused on sagebrush projections at the end of the current century, one important feature of our aproach is that it could in theory be applied to forecast the effects of short-term climate events on sagebrush.  For instance this apprach could be used to model the effects of a multi-year regional drought on this species (cite Germino paper on Yellowstone ecosystem drought).  Validating the model against short-term predictions would give us a way to assess the amount of confidence we should have on any longer-term forecasts produced by the model.  It would also provide opportunities for refinement of the model itself.  This cycle of prediction, validation and refinement is missing from most population-level forecasts of the effects of climate change currently produced.   

Conclusion
----------
We have introduced a new approach to fitting and simulating population models at large spatial extents with state of the art population data derived from remote sensing.
As remote sensing datasets and advances in computing power are made this approach will become useful for a wider number of species and be applicaple at even larger spatial extents, and higher resolution.
