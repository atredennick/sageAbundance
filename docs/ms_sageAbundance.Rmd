...
This resulted in an annual time series of sagebrush cover at 30 meter resolution from 1984 to 2011 (Fig. A1).
...
However, given the relative size of our dataset and computational limitations, we defined kernels around 231 knots (Fig. B2) whose nearest neighbor distances are approximately equal to the range of spatial dependence in residuals from a simple GLM fit without climate covariates and the spatial random effect (~500 meters; Appendix B).
...
Thus, our model predicts an increase in the heterogeneity of sagebrush cover because projected cover increases are smaller in low cover pixels than in high cover pixels (Fig. 5 and Fig. E1).
...
We \hl{introduced} a new approach \hl{that combines a} remotely-sensed time series of plant percent cover [@Homer2012] and methods from the spatio-temporal modeling \hl{literature} [e.g., @Conn2015].
\hl{As a proof-of-concept, we applied our approach to a remotely-sensed data product of sagebrush percent cover from 1984 to 2011 in Wyoming.}
...
An interesting consequence of explicitly modeling the effect of space (through $\boldsymbol{\eta}$) is the forecasted increase in spatial heterogeity (Fig. E1).
...
We were unable to attribute the spatial structure apparent in the data (Fig. 4A) and approximated by our model ($\boldsymbol{\eta}$, Fig. D1) to slope, aspect, elevation, or coarse soil type (results not shown).
...
```{r figure_4,dependson="plot-options", fig.cap="Observed and predicted (A, B) equilibrium percent cover of sagebrush, and prediction bias and precision (C, D) for the extent of our spatial area at 30-m resolution. Observed equilibrium sagebrush cover (A) is the temporal mean of each pixel from the 28 year time series. Prediction results are from simulations that use posteior mean parameter values. Precision in (D) represents the variability of each pixel over the course of the 2,000 iteration simulation. Axes definitions: Lat = latitude; Lon = longitude.", fig.height=8, eval=T}
library(png)
library(grid)
