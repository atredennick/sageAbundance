...
The climate effects detected by our statistical model are in agreement with previous findings at the individual and landscape scale, which imspires some confidence in our forecasts.
...
However, the covariates we chose closely match the climate-related variables that have been shown to drive sagebrush growth, survival, and regeneration (e.g. REFERENCE????).
More likely, aggregated estimates of plant abundance, such as percent cover, mask interannual variability at the level of the individual plant and makes it more difficult to detect the drivers of internanual 
variability. Nonetheless, our model was capabable of detecting climate effects, albeit small, that agree with our knowledge of sagebrush ecology and allowed us to make forecasts of future sagebrush abundance.
...
An interesting consequence of explicitly modeling the effect of space (through $\boldsymbol{\eta}$) is the forecast increase in spatial heterogeity (Fig. D1).
Our model projects little change in low cover pixels but substantial increases in the cover of high cover pixels (Fig. 5). Had we not explicitly accounted for spatial-dependence in our model, we would have missed this result.
