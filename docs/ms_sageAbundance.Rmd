...
However, given the relative largeness of our dataset and computational limitations, we defined kernels around 271 knots (Figure 2) whose nearest neighbor distances are approximately equal to the range of spatial dependence in residuals from a simple GLM fit without climate covariates and the spatial random effect (Appendix A). The Bayesian posterior distribution can be expressed as
...
### Accomodating zeros
Since our process model (Eq. ) includes a log transformation of the observations ($\text{log}(y_{t-1}$), our model does not accomodate zeros. 
