...
Abstract
--------
Plant populations models are powerful tools for predicting climate change impacts in one location, but are difficult to apply at landscape scales.
...
The ideal tool would be a large-scale, dynamic population model [@Schurr2012a; @Merow2014], but developing useful models at this scale has been limited by the availability of time series data at large spatial extents aswell as statistical methods for fitting high-dimensional spatial models.
...
Furthermore, because they lack short-term dynamics they usually cannot produce any estimate of the rate at which local populations will increase or decrease in the near-term and instead project a future equilibrium species distribution that may or may not ever be reached. Furthermore, direct validation of such predictions is extremely rare (cite a paleo paper?).
...
These generated climate time series were used to simulate equilibrium sagebrush cover (see *Simulating the process* below).
We simulated equilibrium cover separately for each GCM and RCP scenario combination, but we show the average projected climate changes across all models in Table 1.

[^link2]: http://cmip-pcmdi.llnl.gov/cmip5/
[^link3]: http://tntcat.iiasa.ac.at/RcpDb/

For the temporally-explicit forecasts we used yearly GCM projections from 2012 to 2098 to make simulate the process starting from the end point of the remotely sensed sagebrush cover data (ends in 2011). 
We aggregated daily GCM output for each GCM and RCP scenario into the yearly climate covariates used to fit our model.
...
We outlined a new approach using remotely-sensed time series of plant percent cover [@Homer2012] and methods from the spatio-temporal modeling of animal abundance [e.g., @Conn2015].
Our approach allowed us to fit a dynamic population model and to make spatially-explicit forecasts of sagebrush response to climate change at the landscape scale.
The climate effects detected by our statistical model are in agreement with previous findings at the individual and landscape scale, which inspires confidence in our forecasts.
More generally, the approach we described is applicable to any spatially-explicit time series of plant population abundance and opens the door to forecasts of plant populations at the landscape-scale.
...
All other covariates related to precipitation and temperature were also positive on average, except for the effect of fall-through-spring precipitation two years before a cover transition (Fig. 3).
These effects, detected at a spatial resolution 30 meters, are consistent with individual-level responses of sagebrush to climate-related variables.
Wetter winters are correlated with greater stem growth in sagebrush [@Poore2009; @Apodaca2013] and warmer spring temperatures may enhance sagebrush growth in colder climates by advancing the date of snowmelt and increasing the length of the growing season [@Perfors2003; @Harte2015].
However, the mean estimates for the climate effects are relatively small on the standardized scale (Fig. 3).
...
Given findings from previous research on the importance of precipitation and temperature on sagebrush growth [@Pechanec1937; @Schlaepfer2011; @Germino2014] and regeneration [@Schlaepfer2014a], it is unlikely that sagebrush are insensitive to climate or that our model is poorly specified.
More likely is that aggregated estimates of plant abundance, like percent cover, mask interannual variability at the level of the individual plant.
Such averaging inevitably reduces interannual variability, which also makes detecting drivers of internanual variability more difficult.
Nonetheless, our model was capabable of detecting climate effects, albeit small, that agree with our knowledge of sagebrush ecology.

Under projected climate, we forecast small positive changes in sagebrush cover in this area for all RCP scenarios (Figs. 5,6).
Our forecast is a direct result of the interaction between the estimated effect size for each cliamte covariate and the amount of change in those covariates projected in the future.
First, precipitation two years before a given year-to-year transition was the most important effect (Fig. 3), but precipitation is projected to increase only moderately (Table 1, Fig. 2).
Second, mean spring temperature had a positive, but small, effect on sagebrush cover changes, while the projected change in temperature was large (Table 1, Fig. 2).
Lastly, the negative effect of precipitation two years before a cover transition (*ppt1*) had an offsetting effect. 

While we forecast an increase in sagebrush cover at our study area, SDM studies tend to project declines in sagebrush abundance in the future due to temperature increases [@Shafer2001; @Neilson2005; @Bradley2010; @Schlaepfer2012; @Still2015].
Why do our results differ?
There are many potential reasons, ranging from the type of model used to the particular climate covariates considered, but the location of our study area in the cold range of the sagebrush distrubtion is the most likely explanation.
The response of plant species to weather varies along climatic gradients [e.g., @Clark2011; @Vanderwel2013b].
Sagebrush are especially sensitive to the timing of snowmelt since their growth depends on recharge of deep soil water [@Schlaepfer2012; @Schlaepfer2014].
In warmer parts the sagebrush range, earlier snowmelt is detrimental to growth and survival [@Pechanec1937; @Schlaepfer2011; @Germino2014].
In colder regions, however, earlier snowmelt due to temperature increases can lengthen the growing season and increase sagebrush occurence and cover [@Schlaepfer2012; @Schlaepfer2014].
Since our study area lies at the cold end of the sagebrush distribution, the weak positive response to temperature detected by our statistical model (Fig. 3) and carried through to our forecasts (Figs. 5,6) likely represents the positive effect of earlier snowmelt.
The positive effect of precipitation we found is consistent with findings from across the sagebrush distribution (CITATIONS), adding further /.//.

Also in contrast to our results, a previous analysis of a different subset of the remote sensing data set we used made projections of sagebrush decline, on average, in the future [@Homer2015].
...
We presented projections for sagebrush cover at the end of this century, but an important feature of our approach is that it could also produce short-term forecasts for sagebrush cover.
For example, we could forecast the effects of a multi-year regional drought on this species [@Debinski2010].
Validating spatial population models against short-term predictions would give ecological forecasters a way to assess the long-term forecasts produced by the model.
It would also provide opportunities for refinement of the model itself.
This cycle of prediction, validation, and refinement is missing from most population-level forecasts of the effects of climate change currently produced.  

### A flexible, landscape-scale plant population modeling approach

Our approach for modeling plant populations overcomes two major hurdles for spatially-explicit population models.
First, we used moderate resolution, remotely-sensed estimates of sagebrush percent cover as a response variable, enabling us to fit a dynamic population model over a large spatial extent. Second, borrowing from new methods in spatio-temporal modeling of animal abundance [e.g., @Conn2015], we fit the model using a dimension reduction strategy that accounted for spatial autocorrelation within a feasible computational time.
...
```{r figure_2a,dependson="plot-options", fig.cap="GCM yearly weather hindcasts (before solid line at 2011) and projections (after solid line at 2011) for precipitation (A) and temperature (B) at our study area in southwestern Wyoming (see Fig. 1).", eval=F}
library(png)
library(grid)
...
```{r figure_4,dependson="plot-options", fig.cap="Observed and predicted (A, B) equilibrium percent cover of sagebrush, and prediction bias and standard deviation (C, D) for the extent of our spatial area at 30-m resolution. Observed equilibrium sagebrush cover (A) is the temporal mean of each pixel from the 28 year time series. Prediction results are from simulations that use mean parameter values and no random year effects. Axes definitions: Lat = latitude; Lon = longitude.", fig.height=8, eval=F}
library(png)
library(grid)
