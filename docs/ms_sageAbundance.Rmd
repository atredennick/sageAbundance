...
Abstract
--------
New remote sensing technology is producing ecological data at unprecedented spatial scales. 
These massive data offer an opportunity to study ecological processes at the landscape and regional scales that have traditionally been observed only at the level of small study plots. 
However, harnessing the power of new spatial data requires statistical models that can both rigorously account for spatial autocorrelation and can also be fit within a reasonable amount of computation time. 
Here we demonstrate how recent advances in the modeling of animal populations can be applied to remotely-sensed, species-specific estimates of plant cover at the landscape scale.
Using modern, computationally efficient model reparameterizations we fit a spatiotemporal population model to a 28 year time series of sagebrush (*Artemisia* spp.) percent cover over a $2.5\times5$ km landscape in southwestern Wyoming with rigorous accounting of spatial autocorrelation.
We include interannual variation in precipitation and temperature as covariates in the model to investigtate how climate affects the cover of this species.
We then use the model to forecast the future abudance of sagebrush at the landscape scale under projected climate change. 
Our analysis provides spatially explicit estimates of change in a plant population that have, until now, been impossible to produce at this scale. 
Critically our large-scale and long-term predictions are rooted in small-scale and short-term population dynamics and so serve as a useful alternative to the predictions offered by most species distribution models that do not include population dynamics at all. 
We expect the approach we develop here will become an indispensable method for using new remote sensing data to model how populations respond to environmental changes, especially climate change, that play out at spatial scales far greater than the average study plot. 
...
Thus, the real challenge is not to simply make population forecasts, but to do so at spatial scales relevant to policy and management decisions [@Queenborough2011].
...
Second, animal population modelers have developed efficient dimension reduction and reparameterization techniques that overcome the dfficulties of fitting high-dimension spatiotemporal models [see @Conn2015 for a review]. 
These new statistical methods have yet to be applied to RS-derived plant populations data at large scales. 

Large-scale, spatially explicit population models based on RS data could offer a valuable new way to investigate the effects of large-scale environmental changes--especially climate change--playing out at landscape and regional scales.
...
Furthermore, because they lack short-term dynamics they usually cannot produce any estimate of the rate at which local populations will increase or decrease in the near-term and instead project a future equilibrium species distribution that may or may not ever be reached within our lifetimes.
Large-scale dynamic population models could overcome both of these limitations.
...
A large-scale spatially-explicit population model for sagebrush driven by interannual climate would provide a valuable new tool for assesing how sagebrush could respond to climate change in the future. 

Building on recent technological advances in spatial statistics and anticipating ever-increasing availability of RS data [@He2015], we demonstrate how large-scale plant population models could be used to predict how climate change will affect sagebrush and other species in the future. 
Our approach relies on a process model motivated by Gompertz density-dependent population growth and a cutting-edge remotely-sensed time series of sagebrush cover from Wyoming [@Homer2012; @Homer2015]. 
We rigorously account for spatial autocorrelation with dimension reduction techniques borrowed from the animal modeling literature [@Conn2015] and produce spatially-explicit estimates of sagebrush percent cover.
...
To demonstrate our modeling approach we use a subset of a remotely-sensed time series of sagebrush (\emph{Artemisia} spp.) shrub percent cover in Wyoming [@Homer2012].
...
The five climate variables in our population model are: (1) "water year" precipitation (*description*) for year \emph{t}-1 (*lagPpt*), (2) year \emph{t}-1 fall through summer precipitation (*ppt2*), (3) year \emph{t}-2 fall through summer precipitation (*ppt1*), (4) year \emph{t}-1 average spring temperature (*TmeanSpr2*), and (5) year \emph{t}-2 average spring temperature (*TmeanSpr1*), where *t-1* to *t* is the transition of interest.
...
To identify top covariates, we could have applied a variety of model selection techniques [@Hooten2015], but we avoid model selection here to maximize focus on the general approach.
...
The CMIP5 provides projections from a suite of global circulations models (GCMs), and we used projections from 18 GCMs (Table A1) that produced weather projections for three ``Representative Concentration Pathways'': RCP 4.5, RCP 6.0, and RCP 8.5 (*described online*)[^link3].
...
We chose these three RCP scenarios to demonstrate our approach because they have close analogues to previous emissions scenarios used by the IPCC.
To simulate equilibrium sagebrush cover under projected future climate, we averaged the 18 GCM projections for each of the three emissions scenarios for the centroid of our study area over the time period of 2050 to 2100.
...
For precipitation, we calculated the proportional change: we subtracted average historic (1950-2000) precipitation from average projected (2050-2100) precipitation, and then divided by average historic (1950-2000) precipitation. 
We calculated precipitation and temperature changes for RCP 4.5, RCP 6.0, and RCP 8.5 scenarios because they are close analogues to previous ICPP scenarios.
...
The spatial random effect ($\boldsymbol{\eta}$) accounts for spatial autocorrelation among pixel that occur closely in space.
Thus, $\boldsymbol{\eta}$ acts as an offset on the intercept ($\beta_{0,t}$) that essentially creates a spatial field that defines how pixels differ from the mean, on average, in space (e.g., areas of perennially low or high cover, relative to average cover).
...
However, given the relative size of our dataset and computational limitations, we defined kernels around 200 knots (Fig. B2) whose nearest neighbor distances are approximately equal to the range of spatial dependence in residuals from a simple GLM fit without climate covariates and the spatial random effect (~500 meters; Appendix B).
...
We assessed convergence visually and calculated scale reduction factors (Appendix C, $\hat{R}$ < 1.1 for all paramters) [@Gelman1992; @Gelman2009]. 
...
Results
-------
Averaged across all GCMs, precipitation and temperature in our study area are projected to increase by 2050-2100 relative to observed climate from 1950-2000, with the magnitude depending on the RCP scenario (Table 1). 
Trajectories of our climate covariates from GCM projections, used to make yearly forecasts, show similar trends (Fig. 2). 
GCM projections are variable for our study area, highlighting the importance of making forecasts based on ensemble projections.

All parameters in our model converged on stable posterior distributions (Appendix C).
Only the *lagPpt* climate covariate can be considered important based on a 90% credible interval, and it had a positive effect on sagebrush percent cover change from year-to-year (Fig. 3).
...
Yet, the need to forecast population responses to climate change over large spatial extents is clear, as demonstrated by the wide application of species distribution models [e.g., @Clark2014].
...
First, we used moderate resolution species-specific estimates of sagebrush percent cover estimated from remote sensing data as a response variable.
Thus, we were able to fit a dynamic population model over a large spatial extent.
...
Second, borrowing from new methods in spatio-temporal modeling of animal abundance [e.g., @Conn2015], we employed a dimension reduction strategy that accounted for spatial autocorrelation while also allowing for feasible computational time in model fitting.
Accounting for spatial autocorrelation in this large dataset allows for statistically rigorous inference on the effects of interannual climate on sagebrush cover change in our study region. 
...
The amount of precipitation in the previous year (*pptLag* in our model) emerged as a strong predictor of sagebrush cover change (Fig. 3), which agrees with previous research showing that wetter winters are correlated with greater stem growth in sagebrush [@Poore2009; @Apodaca2013]. 
The posterior distribution for the effect of spring temperature on sagebrush cover broadly overlapped zero but was slightly positive (Fig. 3).
Warmer spring temperatures may enhance sagebrush growth in colder climates by advancing the date of snowmelt and increasing the length of the growing season [@Perfors2003; @Harte2015].
...
Our model projects only small changes in sagebrush cover in this area with future climate change, which is expected since only precipitation in the previous year emerged as an important covariate (Fig. 3) and projections of future precipitation show only modest increases (Fig. 2).
Under all RCP scenarios, our model projects modest increases in sagebrush cover (Figs. 5,6).
...
A previous analysis of a different subset of the remote sensing data set we used made projections of sagebrush decline, on average, in the future [@Homer2015].
The discrepancy between Homer et al.'s (2015) results and ours reflects a difference in the climate projections used for forecasting future changes rather than differences in our inference about how climate drives sagebrush cover change.
First, Homer et al. (2015) used downscaled IPCC projections that allowed for spatial variation in future climate, unlike our analysis here.
Therefore, Homer et al. (2015) did predict some pixel-level increases in sagebrush cover since each pixel was subject to its own regression between climate and cover.
Second, like our study, Homer et al. (2015) found a generally positive relationship between pixel-level sagebrush cover and precipitation, but the climate scenario they chose resulted in a mean decrease in precipitation.
Thus, the decline in precipitation led to a decline in sagebrush cover.
Our current work shows that when multiple climate projections are considered, on average sagebrush cover is forecast to increase, but decreases are not outside the realm of our forecasts when all sources of uncertainty are considered (Fig. 6).
Lastly, Homer et al.'s (2015) analysis relied on pixel-level regressions of cover through time against climate covariates.
Our current approach is fundamentally different in that we specifically model the impact of weather on year-to-year changes in sagebrush cover.
Thus, our model captures the essential interaction point between populations and the interannual variation in weather that drives population dynamics.

As mentioned above, one of the main differences between our current work and that of Homer et al. (2015) is our choice of climate projections.
Weather projections from GCMs differ because each model incorporates processes and feedbacks in different ways [@Flato2013].
These structural differences among GMCs can lead to divergent weather forecasts for specific regions, even if the GCMs converge on projections at the global scale [@Flato2013].
Thus, there is no clear answer to what climate model to use when making ecological forecasts at the regional scale.
...
In order to overcome this, future investigators modeling a larger spatial extent may need to aggregate their original remote-sensing time series data to a coarse spatial resoluation.
This would allow them to model a much greater spatial extent with a similar number of knots and a similar amount of computation time.
A coarser scale model would lose some fine-scale detail but could be applied to a much larger area, potentially gaining some strength in estimating climate effects by spanning a greater range of climate variation.
However, gains made by incorporating greater regional variability by modeling at a coarser resolution could be offset by the loss of information inherent when aggregating plant responses into larger and larger pixels.
...
We also did not allow for the possibility that different parts of our study area would respond to climate in different ways.
This would require spatially-varying climate effects that substantially increase computational time.
However, in some case, for example over very large regions, allowing the effect of climate to vary over space will be important.
@Conn2015 provide examples of how such spatiotemporal interactions can be included in abundance models.
Future studies could examine lower spatial resolutions and larger spatial extents to facilitate the use of spatially-explicit and spatially-varying climate covariates that match the resolution of the percent cover data.
It is also possible to include relevant abiotic data layers as predictors when fitting models at larger spatial extents where variation in spatial covariates like soil type, slope, and aspect may play more important roles than we observed in our study area.

While we presented projections for sagebrush cover at the end of this century, one important feature of our approach is that it could also produce short-term forecasts for sagebrush cover.
For example, our approach could be used to model the effects of a multi-year regional drought on this species [@Debinski2010].
Validating spatial population models against short-term predictions would give modelers a way to assess the long-term forecasts produced by the model.
It would also provide opportunities for refinement of the model itself.
This cycle of prediction, validation and refinement is missing from most population-level forecasts of the effects of climate change currently produced [@Petchey2015].   
...
Future modeling could include the effects of non-climate drivers -- including the effects of species interactions and disturbance [@Clark2014].
