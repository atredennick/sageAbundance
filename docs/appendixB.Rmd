...
Spatial dependence existed in the residuals until pixels were separated by about 500 meters (Fig. B1). Thus, we set the kernal bandwidth ($\sigma$ in Eq. X of the main text) to 500/3 = `r round(500/3,2)`. We then placed equally-spaced knots across our spatial grid until the distance between nearest neighbors was approximately $\sigma$ = `r round(500/3,2)`. Knots were placed evenly across space in the sense that all pixels are equally-spaced along latitude and longitude, but the latitudinal distance and longitudinal distance are not exactly equal. This was done to achieve a symmetric grid of knots. This resulted in a 11 $\times$ 11 knot grid (121 knots; Fig. B2).

![Spatial variogram of model residuals showing where spatial dependence disappears (dark red line at ~500 meters), and the value chosen for the kernal bandwidth in Eq. X of the main text (500/3; dashed black line).](components/avgresidual_variogram_withrange_subset.png)

![Map of our study area showing percent cover in 1984 and the location of the 121 evenly-spaced knots.](components/SAGE_Grid_wKnots_subset.png)