...
growD[which(growD$Cover==0), "Cover"] <- 0.1
growD[which(growD$CoverLag==0), "CoverLag"] <- 0.1

growD$Cover <- growD$Cover/100
growD$Cover <- growD$CoverLag/100
...
transformed parameters{
  ...
  vector<lower=0>[nobs] A;             // parameter for beta distn
  vector<lower=0>[nobs] B;             // parameter for beta distn
  eta <- K*alpha;
  climEffs <- X*beta;
  for(n in 1:nobs)
    mu[n] <- inv_logit(int_mu + beta_mu*lag[n] + climEffs[n] + eta[cellid[n]]);
  A <- mu * phi;
  B <- (1.0 - mu) * phi;
}
model{
...
mean_params <- ddply(outs, .(Parameter), summarise,
                     mean(value))
alphas <- mean_params[grep("alpha", mean_params$Parameter),2]
betas <- mean_params[grep("beta", mean_params$Parameter),2][2:6]
eta <- K%*%alphas
time.steps <- 100
...
for(t in 2:time.steps){
  ...
  tmp.mu <- mean_params[mean_params$Parameter=="int_mu",2] + mean_params[mean_params$Parameter=="beta_mu",2]*ex.mat[t-1,] + 
    sum(betas*Xtmp)
  tmp.mu <- inv.logit(tmp.mu + eta)
  A <- tmp.mu*mean_params[mean_params$Parameter=="phi",2]
  B <- (1-tmp.mu)*mean_params[mean_params$Parameter=="phi",2]
  ex.mat[t,] <- rbeta(ncol(ex.mat), A, B)
}

matplot(c(1:time.steps), ex.mat*100, type="l", col="grey")
# hist(y, freq = FALSE)
